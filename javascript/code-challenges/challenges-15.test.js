...
const screenForNames = (arr) => {
  // Solution code here...
  // const pattern = (/^(Mr|Mrs|Mr|Dr|Er)\.[A-Za-z]+$/).test(word);
  // return pattern;

  // return
  // (/^(Mr\.|Mrs\.|Ms\.|Dr\.)\s[a-zA-Z]+$/);
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  let biggerMass = arr.filter(element => element.mass > 77);
  let print = biggerMass.map(element => element.name).join(' - ');
